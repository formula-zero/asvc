...
pub fn verify_upk<E: PairingEngine> (
  ...
  let inner = vrk_params.list_g2_tau_i[1].into_projective().sub(
    &vrk_params.list_g2_tau_i[0].into_projective().mul(omega_i.into()),
  );
...
pub fn update_commit<E: PairingEngine> (
  ...
  let new_commit = commit.commit.into_projective().add(&(l_value.mul(delta.into())));
